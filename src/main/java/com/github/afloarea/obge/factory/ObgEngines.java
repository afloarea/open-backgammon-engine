...
public final class ObgEngines {
    ...
    public static InteractiveObgEngine newInteractive(BoardTemplate template) {
        return new BasicObgEngine(ColumnsFactory.buildStartingSequence(template));
    }

    public static TurnBasedObgEngine newTurnBased(BoardTemplate template) {
        return new PredictingObgEngine(ColumnsFactory.buildStartingSequence(template));
    }

    public static MixedModeObgEngine newMixed(BoardTemplate template) {
        return new PredictingObgEngine(ColumnsFactory.buildStartingSequence(template));
    }
